...
ShapeWeaver is a runnable MVP web app for teaching and learning solid geometry. It supports two simple workflows:
...
- OCR service with graceful fallback when `pytesseract` or the Tesseract binary is unavailable
...
- Backend: Python, FastAPI, OpenCV, pytesseract, Pydantic
...
```bash
cd /Users/qingshi/Desktop/Project/GeoWeaver/backend
python3 -m venv .venv
...
```bash
cd /Users/qingshi/Desktop/Project/GeoWeaver/frontend
npm install
```
...
```bash
cd /Users/qingshi/Desktop/Project/GeoWeaver/backend
./.venv/bin/python -c "import fastapi, cv2, pytesseract; from app.main import app; print('backend-import-ok')"
```
...
`pytesseract` is optional at runtime. If the Python package or the system Tesseract binary is unavailable, the backend will not crash. Instead it returns a warning such as:

`OCR unavailable in current environment; please enter or edit the problem text manually.`

To enable OCR fully, install:
...
You can copy [`backend/.env.example`](/Users/qingshi/Desktop/Project/GeoWeaver/backend/.env.example) and fill those values locally.
